...
| Map | **MapLibre GL JS** + **Protomaps `.pmtiles`** basemap served as a static file | No API keys, no per-tile billing, no vendor account for the church to manage, and a single tile file caches beautifully in a service worker. **Built** — `web/public/basemap/blacksburg.pmtiles`, 4.4 MB, ~50 km around the town. The corpus is public-domain USGS rather than the usual OpenStreetMap build, which keeps the ODbL away from this repository entirely (docs/01 §2.4). See docs/20 §2. |
