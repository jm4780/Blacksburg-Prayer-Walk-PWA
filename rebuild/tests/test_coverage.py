...
Everything here runs against the real 1,553-segment Blacksburg network in
rebuild/data/out/network.geojson. Walks follow real segment geometry; noise is
a correlated (AR(1)) two-dimensional error, which is what consumer GPS actually
does -- white noise averages out over a minute and would make these tests far
easier than reality.
...
def segs(raw):
    ...
    for f in raw["features"]:
        p = dict(f["properties"])
        p["line"] = LineString([tr.transform(lo, la) for lo, la in f["geometry"]["coordinates"]])
        out[p["seg_id"]] = p
    return out
...
# Real Blacksburg walks, verified node-continuous against the shipped network.
#
# CLEAN: Palmer Dr -> Palmer Dr -> Graves Ave -> Graves Ave, 556 m. Reynolds
#        Ave (1251) runs parallel to Graves Ave 72 m away and shares a junction
#        with both Palmer Dr segments, so it is topologically reachable and is
#        exactly the wrong answer a nearest-line matcher would give.
CLEAN_PATH = [1249, 1252, 1256, 1254]
CLEAN_TRAP = 1251

# DOWNTOWN: Otey St -> W Roanoke St -> Draper Rd SW -> Draper Rd SW, 455 m.
#        Wall St (1111) is 46-53 m from W Roanoke St, 2.3 degrees off parallel,
#        and shares a junction with the Draper Rd segment the walker turns onto.
#        This is the pair the contract's "~60 m apart" clause is about.
DOWNTOWN_PATH = [1102, 1108, 1110, 1112]
DOWNTOWN_TRAPS = [1111, 1113]

# LONG: 1.2 km through the middle of downtown, used for the dropout case and
#        for the false-positive sweep.
LONG_PATH = [1102, 1108, 1105, 1081, 920, 921, 937, 1141, 1154, 1145, 1143]

DROPOUT_PATH = [1249, 1252, 1256, 1257, 1258]
...
def path_polyline(segs, seg_ids):
    ...
    previous one ended at. Raises if the path is not node-continuous, so a
    typo'd test path fails loudly instead of silently teleporting.
    """
    first, second = segs[seg_ids[0]], segs[seg_ids[1]]
    shared = {first["node_a"], first["node_b"]} & {second["node_a"], second["node_b"]}
    assert len(shared) == 1, f"segments {seg_ids[:2]} are not simply adjacent"
    cur = ({first["node_a"], first["node_b"]} - shared).pop()
...
def random_walks(segs, count, *, target_m=600.0, seed=7):
    ...
    Three hand-picked paths can be tuned against by accident. These cannot: the
    walker wanders a random connected route through whatever Blacksburg
    actually looks like there -- cul-de-sacs, hairpins, duplicated geometry and
    all -- which is what the headline false-positive number needs to survive.
    """
    adjacency = {}
    ...
    while len(out) < count and attempts < count * 60:
        ...
        while total < target_m:
            opts = [x for x in adjacency.get(cur, ()) if x not in used and usable(x)]
            if not opts:
                break
            nxt = rng.choice(opts)
            used.add(nxt)
...
TICKED = 0.5        # contract rule 6: below this the confirm screen shows unticked
...
def test_clean_walk_proposes_the_streets_walked(net, segs, to_lonlat):
    walked = set(CLEAN_PATH)
    for seed in range(6):
        trace = simulate(segs, to_lonlat, CLEAN_PATH, accuracy_m=8.0, seed=seed)
        ...
        # And specifically not the parallel street that shares their junction.
        assert CLEAN_TRAP not in on
        trap = next((p for p in props if p["seg_id"] == CLEAN_TRAP), None)
        assert trap is None or trap["confidence"] < TICKED
        ...
        walked_m = sum(segs[s]["line"].length for s in CLEAN_PATH)
        got_m = sum(p["matched_m"] for p in on.values())
        assert got_m / walked_m >= 0.75, f"seed {seed}: only {got_m:.0f}/{walked_m:.0f} m"

        # The two full interior segments are unambiguous; they must be ticked.
        assert {1252, 1256} <= set(on), f"seed {seed}: missed interior segments"


def test_clean_walk_matched_m_is_bounded_by_reality(net, segs, to_lonlat):
    """Never claim more metres of a segment than the segment has."""
    trace = simulate(segs, to_lonlat, CLEAN_PATH, accuracy_m=8.0, seed=11)
...
def test_downtown_parallel_streets_are_never_proposed(net, segs, to_lonlat):
    """W Roanoke St vs Wall St: 46-53 m apart, 2.3 degrees off parallel.

    The noise here regularly puts fixes closer to Wall St than to the street
    the walker is on, so a nearest-line matcher fails this outright. Topology
    plus the accuracy-weighted emission has to carry it.
    """
    walked = set(DOWNTOWN_PATH)
    worst_gap = 1.0
    for seed in range(8):
        trace = simulate(segs, to_lonlat, DOWNTOWN_PATH, accuracy_m=25.0, seed=100 + seed)
        props = record(f"downtown/{seed}", walked, propose(trace, net))
        on = ticked(props)

        assert not (set(on) - walked), (
            f"seed {seed}: proposed unwalked "
            f"{[(s, props_for(props, s)) for s in set(on) - walked]}")
        for trap in DOWNTOWN_TRAPS:
            assert trap not in on, f"seed {seed}: proposed parallel street {trap}"
            t = next((p for p in props if p["seg_id"] == trap), None)
            if t is not None:
                worst_gap = min(worst_gap, TICKED - t["confidence"])

        # W Roanoke St itself is the street under test; it must still be found.
        assert 1108 in on, f"seed {seed}: lost the street actually walked"
...
def test_a_trace_hugging_the_parallel_street_still_refuses_to_guess(net, segs, to_lonlat):
    """Bias the whole trace 30 m toward Wall St and demand it stays honest.

    The right answers here are "W Roanoke St, unsure" or "nothing". The one
    unacceptable answer is a ticked Wall St.
    """
    trace = simulate(segs, to_lonlat, DOWNTOWN_PATH, accuracy_m=25.0, seed=77)
    # 30 m of constant sideways bias, roughly perpendicular to W Roanoke St.
    biased = []
    for f in trace:
        biased.append({**f, "lat": f["lat"] + 30.0 / 111_320.0})
    props = record("downtown-biased", set(DOWNTOWN_PATH), propose(biased, net))
    on = ticked(props)
    for trap in DOWNTOWN_TRAPS:
        assert trap not in on, f"biased trace ticked parallel street {trap}"
    assert not (set(on) - set(DOWNTOWN_PATH))
...
def test_dropout_matches_both_sides_and_invents_nothing(net, segs, to_lonlat):
    walked = set(DROPOUT_PATH)
    spans = segment_spans(segs, DROPOUT_PATH)
    line = path_polyline(segs, DROPOUT_PATH)

    # A three-minute hole starting exactly once the first segment is behind us,
    # so the second segment is swallowed whole.
    gap_start_m = spans[DROPOUT_PATH[1]][0]
    ...
    # Which segments were the walker's phone actually awake for?
    observed_m = {}
    for sid, (a, b) in spans.items():
        seen = max(0.0, min(b, gap_start_m) - a) + max(0.0, b - max(a, gap_end_m))
        observed_m[sid] = seen
    swallowed = [s for s, m in observed_m.items() if m < 10.0]
    assert swallowed, "the hole should swallow at least one whole segment"

    for seed in range(5):
        full = simulate(segs, to_lonlat, DROPOUT_PATH, accuracy_m=8.0, seed=200 + seed)
        ...
        # Both sides of the hole are matched. (Which segments after the hole
        ...
        assert DROPOUT_PATH[0] in on, f"seed {seed}: lost the pre-dropout segment"
        post_gap = [s for s in DROPOUT_PATH if spans[s][0] >= gap_end_m]
        assert post_gap
        assert any(s in on and on[s]["matched_m"] > 100.0 for s in post_gap), (
            f"seed {seed}: nothing matched after the dropout")
...
def test_dropout_is_not_bridged_even_when_geometry_would_allow_it(net, segs, to_lonlat):
    """Two fixes 10 minutes apart on the same street are two walks, not one."""
    trace = simulate(segs, to_lonlat, CLEAN_PATH, accuracy_m=8.0, seed=31)
    head = trace[:6]
    ...
    assert total <= walked_head_tail + 60.0, (
        f"bridged the 10-minute gap: claimed {total:.0f} m")
...
def test_partial_final_segment_is_proposed_but_low_confidence(net, segs, to_lonlat):
    spans = segment_spans(segs, CLEAN_PATH)
    last = CLEAN_PATH[-1]
    last_a, last_b = spans[last]
    half = last_a + (last_b - last_a) * 0.5

    for seed in range(5):
        trace = simulate(segs, to_lonlat, CLEAN_PATH, accuracy_m=8.0,
                         seed=300 + seed, stop_at_m=half)
        props = record(f"suspended/{seed}", set(CLEAN_PATH), propose(trace, net))
        ...
        on = ticked(props)
        assert not (set(on) - set(CLEAN_PATH))
        assert {1252, 1256} <= set(on), f"seed {seed}: lost the completed segments"
...
@pytest.mark.parametrize("seg_id", [1256, 1108, 1252])
def test_standing_still_does_not_paint_streets(net, segs, to_lonlat, seg_id):
    for seed in range(4):
        trace = simulate_stationary(segs, to_lonlat, seg_id,
                                    accuracy_m=30.0, seed=400 + seed)
        assert len(trace) > 100
        # The walker stood on `seg_id`; they walked nothing.
        ...
        assert sum(p["matched_m"] for p in props) < 80.0, (
            f"seg {seg_id} seed {seed}: claimed {props}")


def test_walk_then_long_stop_credits_only_the_walk(net, segs, to_lonlat):
    ...
    """
    walk = simulate(segs, to_lonlat, CLEAN_PATH, accuracy_m=10.0, seed=55)
    stop_start = walk[-1]["t"] + SAMPLE_DT_S
    stand = simulate_stationary(segs, to_lonlat, CLEAN_PATH[-1], accuracy_m=25.0,
                                seed=56, minutes=5.0, t0=stop_start)
    props = record("walk-then-stop", set(CLEAN_PATH), propose(walk + stand, net))
    on = ticked(props)
    assert not (set(on) - set(CLEAN_PATH))
    walked_m = sum(segs[s]["line"].length for s in CLEAN_PATH)
...
def test_proposal_shape_and_purity(net, segs, to_lonlat):
    trace = simulate(segs, to_lonlat, DOWNTOWN_PATH, accuracy_m=12.0, seed=9)
    ...
    assert props == sorted(props, key=lambda p: (-p["confidence"], -p["matched_m"], p["seg_id"]))
...
def test_low_quality_fixes_do_not_outvote_good_ones(net, segs, to_lonlat):
    """Contract rule 4. Same walk, better fixes must not be less certain."""
    good = propose(simulate(segs, to_lonlat, CLEAN_PATH, accuracy_m=8.0, seed=5), net)
    poor = propose(simulate(segs, to_lonlat, CLEAN_PATH, accuracy_m=45.0, seed=5), net)
    g = {p["seg_id"]: p["confidence"] for p in good}
    p_ = {p["seg_id"]: p["confidence"] for p in poor}
    for sid in (1252, 1256):
        assert g.get(sid, 0.0) >= p_.get(sid, 0.0), sid
    # And the poor trace must not have gone on to invent streets.
    assert not ({s for s, c in p_.items() if c >= TICKED} - set(CLEAN_PATH))


def test_heading_rules_out_the_cross_street(net, segs, to_lonlat):
    """Rule 3. Walk Graves Ave; the cross street at its junction is not walked."""
    trace = simulate(segs, to_lonlat, [1256, 1254], accuracy_m=10.0, seed=13)
    on = ticked(propose(trace, net))
    # Evergreen Way (1257) leaves the same junction at right angles.
    assert 1257 not in on
    assert not (set(on) - {1256, 1254})
...
def test_false_positive_rate_across_all_traces(net, segs, to_lonlat, capsys):
    ...
    realistically sees, and folds in every record the tests above produced.
    """
    records = list(_RECORDS)

    scenarios = [("clean", CLEAN_PATH), ("downtown", DOWNTOWN_PATH), ("long", LONG_PATH)]
    ...
    for name, path in scenarios:
        for acc in (8.0, 15.0, 25.0, 40.0, 60.0):
            for seed in range(2 if name.startswith("random") else 5):
                ...
                rec = (f"sweep/{name}/{acc:g}/{seed}", set(path), propose(trace, net))
                records.append(rec)
    ...
    parallel = [r for r in records if "downtown" in r[0] or "clean" in r[0]]
    p_rate, p_tp, p_fp, p_off = fp_rate(parallel)
    usable = [r for _a, rs in by_acc.items() if _a <= 25.0 for r in rs]
