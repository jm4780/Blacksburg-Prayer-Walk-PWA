...
web/src/map/ the Prayer Walk map design system — tokens, contexts, layer order.
             The basemap is ours: a self-hosted Protomaps .pmtiles archive of the
             region around Blacksburg, built from public-domain USGS data and
             styled in this system's own palette, not a vendor's tiles (docs/20)
pipeline/basemap/  how that archive is rebuilt from USGS downloads
...
- [`docs/20-map-design-system.md`](docs/20-map-design-system.md) — **current**: the cartographic language, and the self-hosted `.pmtiles` basemap it is drawn on. The world continues past the town limit in every direction, from public-domain USGS data, with the prayer overlays leading by contrast rather than by weight. Specimen sheet at `#/map-system`.
